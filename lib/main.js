...
MySpreadsheet.prototype.getAllRows = function(callback) {
  this.sheetReceive(function(err, rows, info) {
    if (err) throw err;
    rows = _.omit(rows, 1); //remove first row(header) using _omit
    callback(null, _constructJsonObj(_getHeaderRow(rows), rows));
  });
...
MySpreadsheet.prototype.getRowByValue = function(properties, callback) {
  this.getRows(function(err, result) {
    callback(null, _.findWhere(result, properties));
  })
}
...
MySpreadsheet.prototype.insert = function(params, callback) {
  this.sheetReady(function(err, spreadsheet) {
    ...
    spreadsheet.send(function(err, result) {
      callback(err, "Row added successfully");
    });
...
MySpreadsheet.prototype.addNewRow = function(params, callback) {
  ...
  this.sheetReceive(function(err, rows, info) {
    ...
    newRow[info.nextRow] = obj;

    console.log("..newRow newRow....", newRow);

    _self.insert(newRow, callback)
  })
}
